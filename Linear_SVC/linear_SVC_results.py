...
def main():
    ...
    """
    Construction of the Pipeline for Linear SVC model with the GridsearchCV results included.
    CalibratedClassifierCV below helps to generate predict probabilities
    for Linear SVC model because svm.LinearSVC doesn't have predict_proba function.
    """
    linSVC_probab_pipeline = Pipeline([
        ('vectpip', vect_pipeline),
        ('cclf', CalibratedClassifierCV(base_estimator=svm.LinearSVC(C=1, loss='hinge', multi_class='crammer_singer', \
                                                                     penalty='l1', dual=False, max_iter=1200), cv=5,
                                        method='sigmoid')),
    ])
    # Fit Linear SVC model according to train data
    linSVC_probab_pipeline.fit(X, y)

    # Get the predicted codes for training set, validation and test set
    predicted_train = linSVC_probab_pipeline.predict(X)
    predicted_test = linSVC_probab_pipeline.predict(X_test)
    predicted_valid = linSVC_probab_pipeline.predict(X_valid)
    ...
    # Let's summarize and print all the results for the LinearSVC classifier according to the dataset sort
    print("\nGeneral metrics for")
    print("TRAINING DATA")
    print("=" * 30)
    ...
    # dump(linSVC_probab_pipeline, 'linearSVC.joblib')
    ...
    predicted_prob = linSVC_probab_pipeline.predict_proba(X_test)
    df_test['Probability'] = predicted_prob.max(axis=1)
    df_test.to_excel("linSVC_autocode.xlsx")
